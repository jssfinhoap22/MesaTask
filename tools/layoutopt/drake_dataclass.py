...
class SceneVecDescription:
    ...
    def __init__(
        ...
        model_paths: List[str],
        model_path_vec_len: Union[int, None],
        welded_object_model_paths: List[str] = None,
    ):
        self.drake_package_map = drake_package_map
        self.static_directive = static_directive
        self.translation_vec_len = translation_vec_len
        self.rotation_parametrization = rotation_parametrization
        self.model_paths = model_paths
        self.model_path_vec_len = model_path_vec_len
        self.welded_object_model_paths = welded_object_model_paths or []
    ...
    @classmethod
    def from_json(cls, json_data: dict, drake_package_map: PackageMap, static_directive: str = None) -> 'SceneVecDescription':
        """
        Create scene description from JSON format.
        """
        # Extract all unique model paths
        model_paths = []
        welded_paths = []
        for obj in json_data["objects"]:
            ...
            if obj.get("is_welded", False):
                welded_paths.append(model_path)

        # Create scene description object
        return cls(
            drake_package_map=drake_package_map,
            static_directive=static_directive,
            translation_vec_len=3,  # [x, y, z]
            rotation_parametrization=RotationParametrization.QUATERNION,  # Use quaternion
            model_paths=model_paths,
            model_path_vec_len=len(model_paths) + 1,  # +1 for empty object
            welded_object_model_paths=welded_paths
        )

    def json_to_scene_tensor(self, json_data: dict) -> torch.Tensor:
        """
        Convert scene data from JSON format to tensor format.

        Args:
            json_data (dict): Scene data in JSON format

        Returns:
            torch.Tensor: Scene tensor with shape (N, D), where N is number of objects and D is feature dimension
        """
        ...
        for obj in objects:
            ...
            rotation = torch.tensor(obj["rotation"], dtype=torch.float32)  # quaternion

            # Create one-hot vector for model path, with extra dimension for empty object
            model_path = obj["model_path"]
            model_path_idx = self.model_paths.index(model_path)
            model_path_vec = torch.zeros(len(self.model_paths) + 1)  # +1 for empty object
            model_path_vec[model_path_idx] = 1

            # Combine all features
            scene_tensor = torch.cat([position, rotation, model_path_vec])
            scene_tensors.append(scene_tensor)

        return torch.stack(scene_tensors)

    def scene_tensor_to_json(self, scene_tensor: torch.Tensor) -> dict:
        """
        Convert scene tensor back to JSON format.

        Args:
            scene_tensor (torch.Tensor): Scene tensor

        Returns:
            dict: Scene data in JSON format
        """
        objects = []
        
        for obj_tensor in scene_tensor:
            # Extract position and rotation
            position = obj_tensor[:3].tolist()
            rotation = obj_tensor[3:7].tolist()  # quaternion

            # Get model path
            model_path_vec = obj_tensor[7:]
            model_path_idx = torch.argmax(model_path_vec).item()
            model_path = self.model_paths[model_path_idx]

            # Check if object is welded
            is_welded = model_path in self.welded_object_model_paths
